...
std::wstring win_text::WidenFromUtf8(const std::string& str)
{
    return Widen(str.c_str(), static_cast<int>(str.size()), CCodePage::kUtf8);
}

std::wstring win_text::WidenFromUtf8(const char* str, int length)
{
    return Widen(str, length, CCodePage::kUtf8);
}

std::string win_text::NarrowToUtf8(const std::wstring& wstr)
{
    return Narrow(wstr.c_str(), static_cast<int>(wstr.size()), CCodePage::kUtf8);
}

std::string win_text::NarrowToUtf8(const wchar_t* wstr, int length)
{
    return Narrow(wstr, length, CCodePage::kUtf8);
}

std::wstring win_text::WidenFromAnsi(const std::string& str)
{
    return Widen(str.c_str(), static_cast<int>(str.size()), CCodePage::kAnsi);
}

std::wstring win_text::WidenFromAnsi(const char* str, int length)
{
    return Widen(str, length, CCodePage::kAnsi);
}

std::string win_text::NarrowToAnsi(const std::wstring& wstr)
{
    return Narrow(wstr.c_str(), static_cast<int>(wstr.size()), CCodePage::kAnsi);
}

std::string win_text::NarrowToAnsi(const wchar_t* wstr, int length)
{
    return Narrow(wstr, length, CCodePage::kAnsi);
}

int win_text::WidenFromUtf8InBuffer(const char* str, int length, wchar_t* dst, int dstSize)
{
    return ::MultiByteToWideChar(CP_UTF8, 0, str, length, dst, dstSize);
}

int win_text::NarrowToUtf8InBuffer(const wchar_t* wstr, int length, char* dst, int dstSize)
{
