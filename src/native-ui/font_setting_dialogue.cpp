...
CFontSettingDialogue::CFontSettingDialogue()
{
	...
	m_hFont = ::CreateFontW(iFontHeight, 0, 0, 0, FW_REGULAR, FALSE, FALSE, FALSE, EASTEUROPE_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, L"Yu mincho");
}
...
LRESULT CFontSettingDialogue::OnInit(HWND hWnd)
{
	...
	if (m_pTextWriter != nullptr)
	{
		...
		if (sBuffer[0] != 'L\0')
		{
			...
			{
				/*実行環境の言語・文字による表記で再探索。*/
				std::wstring wstrLocaleFontName = winFont.FindLocaleFontName(sBuffer);
...
void CFontSettingDialogue::OnApplyButton()
{
	...
	if (!wstrFontName.empty())
	{
		...
		std::vector<std::wstring> filePaths = winFont.FindFontFilePaths(wstrFontName.c_str(), bBold, bItalic);
		if (!filePaths.empty())
		{
			...
			CD2TextWriter* pD2TextWriter = static_cast<CD2TextWriter*>(m_pTextWriter);
			pD2TextWriter->SetFontByFontName(wstrFontName.c_str(), winFont.GetLocaleName(), bBold, bItalic);
